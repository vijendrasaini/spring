...
public  interface EmployeeRepository extends JpaRepository<EmployeeEntity, Integer>{
    ...
    // @EntityGraph(value = "Employee.withDepartment") // when using named entity graph
    ...
    //     nativeQuery = true
    // )
    // Page<EmployeeEntity> findBySalaryGreaterThanNative(@Param("minSalary") BigDecimal minSalary, Pageable pageable);
  ...
  // Day 22 bonus — native JOIN + DTO projection (no lazy department, no N+1)
    ...
    // Native + Page: do NOT add LIMIT/OFFSET — pass Pageable; Spring appends pagination + count
    @Query(
        value = """
            SELECT e.id AS id,
                   e.name AS name,
